var EtherTDD = function(web3, promiselib, BLOCK_DELAY) {
    ...
    var Contract = function() {
        ...
        var create = function(options, callback) {
            ...
            if (contractPath in _cache) {
                options.data = _cache[contractPath].data
                _createContract(_cache[contractPath].abi, options, callback);
                return promise;
            }   
            ...
            get(contractPath + '.binary', function(hex) {    
                ...
                get(contractPath + '.abi', function(abi) {
                    abi = JSON.parse(abi);
                    _cache[contractPath] = {'data': options.data, 'abi': abi};
                    _createContract(abi, options, callback);
                });
            });
